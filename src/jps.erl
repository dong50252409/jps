...
search(StartGrid, EndGrid, ValidFun, Options) ->
    ...
    {OpenGrids1, VisitedGrids1} = add_jump_grids(EndGrid, StartGrid, 0, [StartGrid], OpenGrids, VisitedGrids, JumpGrids),
%%    draw_map(OpenGrids1),
    MaxLimit = proplists:get_value(max_limit, Options, ?DEFAULT_MAX_LIMIT),
    do_search(EndGrid, ValidFun, OpenGrids1, VisitedGrids1, MaxLimit).
    ...
    ValidFun =
        fun({X, Y}) ->
            X =< 10 andalso Y =< 10 andalso 0 < X andalso 0 < Y
                andalso element(X, element(Y, Map)) =/= $X
        end,
